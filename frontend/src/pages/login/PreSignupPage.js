...
function PreSignupPage() {
  ...
  const preNext = async (e) => {
    ...
    try {
      const res = await axios.get(`http://3.37.151.29:8080/api/members/email_check`, {
        params: { email: email },
      });
  ...
  const handleGoogleSignup = () => {
    window.location.href = "http://3.37.151.29:8080/oauth2/authorization/google";
  };

  // 주석: 카카오 회원가입
  const handleKakaoSignup = () => {
    window.location.href = "http://3.37.151.29:8080/oauth2/authorization/kakao";
  };
  return (
    <div className={styles.signup_container}>
      <div className={styles.signup_smallcontainer}>
        ...
        <h2 className={styles.signup_title}>일반 회원가입</h2>
        ...
        <form className={styles.signup_form} onSubmit={preNext}>
          ...
          {/* 추가데이터 입력 */}
          <button type="submit" className={styles.pre_signup_button}>
            계속
          </button>
          ...
          <button type="button" className={styles.social_signup_g} onClick={handleGoogleSignup}>
            <img className={styles.socialicon} src={socialg}></img>
            구글 계정으로 회원가입
          </button>

          <button type="button" className={styles.social_signup_k} onClick={handleKakaoSignup}>
            <img className={styles.socialicon} src={socialk}></img>
            카카오 계정으로 회원가입
          </button>
        </form>
      </div>
