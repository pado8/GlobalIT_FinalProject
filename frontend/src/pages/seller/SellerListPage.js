...
const SellerListPage = () => {
  ...
  useEffect(() => {
    if (modal_open) {
      ...
      document.body.style.marginRight = `${scrollBarWidth}px`; // scrollbar 공간 확보
    } else {
    ...
    <div className={styles["page_wrapper"]}>
      <div className={styles["wrap_container"]}>
        <div className={styles["header"]}>
          ...
          <p className={styles["page_subtitle"]}>고객 요청에 맞춰 입찰한 업체들을 빠르게 비교해보세요.</p>
                    ...
                    {Array.isArray(simage) && simage.length > 1 && simage.slice(1).some((img) => img?.trim()) && (
                      <div className={styles["image_slider"]}>
                        {simage.length > 4 && slide_index > 0 && (
                          <button onClick={() => setSlideIndex((prev) => Math.max(prev - 3, 0))} className={styles["slider_button"]}>
                            {"<"}
                          </button>
                        )}
                        ...
                        {simage
                          .slice(1)
                          .slice(slide_index, slide_index + 3)
                          .filter((img) => img?.trim())
                          .map((img, i) => (
                        ...
                        {simage.length > 4 && slide_index + 3 < simage.length - 1 && (
                          <button onClick={() => setSlideIndex((prev) => prev + 3)} className={styles["slider_button"]}>
                            {">"}
                          </button>
                        )}
                      </div>
                          ...
                          {Array.isArray(reviews) && reviews.length === 0 ? (
                            ...
                            <ul className={styles["review_list"]}>
                              {reviews.map((rev, idx) => {
                                ...
                                return (
                                  <li key={idx} className={styles["review_item"]} ref={isLast ? lastReviewElementRef : null}>
                                    <div className={styles["review_rating"]}>{renderStars(rev.rating)}</div>
                                    <div className={styles["review_author"]}>작성자: {rev.username || "익명"}</div>
                                    <div className={styles["review_content"]}>{rev.rcontent}</div>
                                    <div className={styles["review_date"]}>{formatDate(rev.regDate)}</div>
                                  </li>
                                );
                              })}
                            </ul>
          ...
          <div className={styles["modal_overlay"]} onClick={() => setEnlargedImage(null)}>
            <div className={styles["modal_content"]} onClick={(e) => e.stopPropagation()}>
              ...
              <div className={`${styles["modal_body"]} ${styles["centered_image_body"]}`}>
                <img src={enlarged_image} alt="확대 이미지" className={styles["enlarged_image"]} />
              </div>
            </div>
          </div>
        )}
